...
/** Bigger character slots for better visibility on the board */
const SLOT_CLASSES = 'h-[76px] w-[55px] sm:h-[88px] sm:w-[64px] md:h-[100px] md:w-[72px]';
...
export function CharacterSlot({ character, isOwn, isSelectable, onClick, onInspect }: CharacterSlotProps) {
  ...
  if (character.hidden && !isOwn) {
    // Opponent's hidden card — Naruto-themed card back
    return (
      <button
        type="button"
      ...
      >
        ...
        <span className="absolute bottom-1 text-[7px] font-medium tracking-wider text-orange-400/50 uppercase sm:text-[8px]">
          {t('game.hidden')}
        </span>
      </button>
    );
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center gap-0.5">
          ...
          {isSelectable ? (
            ...
            <div className="flex items-center gap-0.5 rounded-full bg-amber-500/20 px-1.5 py-0.5">
              <Eye className="h-2.5 w-2.5 text-amber-400" />
              <ChakraIcon className="h-2.5 w-2.5 text-blue-400" />
            ...
            <>
              ...
              <span className="text-[7px] tracking-wide text-amber-500/60 uppercase sm:text-[8px]">{t('game.hidden')}</span>
            </>
          )}
        </div>
      </div>
  ...
  // Visible character — card image with stats overlay
  return (
    <div
      {...longPressHandlers}
      role="button"
    ...
    >
      {imageUrl ? (
        ...
        <div className={cn(
          ...
          <span className={cn(
            'max-w-full truncate text-[8px] font-semibold leading-tight sm:text-[9px]',
          ...
          <div className="flex items-center gap-1">
            <div className="flex items-center gap-0.5">
              ...
              <span className="text-[10px] font-bold text-red-400">{character.card.power}</span>
            </div>
          </div>
        </div>
      )}

      {/* Dark gradient overlay — bottom stats */}
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/95 via-black/70 to-transparent px-1 pb-1 pt-5">
        ...
        <div className="mt-0.5 flex items-center gap-2">
          {/* Power stat — red like the TCG cards */}
          <div className="flex items-center gap-0.5">
            <PowerIcon className={cn(
              'h-3 w-3 drop-shadow-[0_0_2px_rgba(0,0,0,0.5)]',
              character.powerTokens > 0 ? 'text-green-400' : 'text-red-400'
            )} />
            <span className={cn(
          ...
          {/* Chakra stat — blue flame */}
          <div className="flex items-center gap-0.5">
            <ChakraIcon className="h-2.5 w-2.5 text-blue-400 drop-shadow-[0_0_2px_rgba(59,130,246,0.5)]" />
      ...
      {character.powerTokens > 0 && (
        <div className="absolute right-0.5 top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-green-500 text-[9px] font-bold text-white shadow-lg animate-power-pop">
      ...
      {character.continuousEffects.length > 0 && (
        <div className="absolute left-0.5 top-0.5 z-10">
          <Sparkles className="h-3.5 w-3.5 text-violet-400 drop-shadow-[0_0_3px_rgba(139,92,246,0.6)]" />
        </div>
      )}
